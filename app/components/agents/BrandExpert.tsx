...
export default function BrandExpert() {
  ...
  const downloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const fileName = (isSetup ? "Brand_Setup_Report" : "Brand_Weekly_Update") + "_" + now.toISOString().slice(0,10) + ".pdf";

      const doc = await buildPDF({
        theme:    THEMES.brand,
        title:    isSetup ? "Brand Setup Report" : "Weekly Update Report",
        subtitle: isSetup ? `Vollständige Markenbasis · ${dateStr}` : `KW ${kw} · ${dateStr}`,
        date:     dateStr,
